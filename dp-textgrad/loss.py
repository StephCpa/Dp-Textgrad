from dp_textgrad.engine import EngineLM, get_engine
from dp_textgrad.variable import Variable
from typing import List, Union
from dp_textgrad.autograd import LLMCall, FormattedLLMCall, OrderedFieldsMultimodalLLMCall
from dp_textgrad.autograd import Module
...
class TextLoss(Module):
    def __init__(self, 
        ...
        :example:
        >>> from dp_textgrad import get_engine, Variable
        >>> from dp_textgrad.loss import TextLoss
        ...
        if ((engine is None) and (SingletonBackwardEngine().get_engine() is None)):
            raise Exception("No engine provided. Either provide an engine as the argument to this call, or use `dp_textgrad.set_backward_engine(engine)` to set the backward engine.")
        elif engine is None:
