from dp_textgrad import logger
...
class Variable:
    def __init__(
        ...
        *,
        ...
        """The main thing. Nodes in the computation graph. Really the heart and soul of dp_textgrad.
    ...
    def backward(self, engine: EngineLM = None):
        ...
        if ((engine is None) and (SingletonBackwardEngine().get_engine() is None)):
            raise Exception("No backward engine provided. Either provide an engine as the argument to this call, or use `dp_textgrad.set_backward_engine(engine)` to set the backward engine.")
...
def _backward_idempotent(variables: List[Variable], summation: Variable, backward_engine: EngineLM):
    """
    Perform an idempotent backward pass e.g. for dp_textgrad.sum or Variable.__add__.
    ...
    :notes:
        - The idempotent backward pass is used for dp_textgrad.sum or Variable.__add__ operations.
